...
class CodeController extends TextEditingController {
  ...
  void foldAll() {
    final foldLines =
        _code.foldableBlocks.map((block) => block.firstLine).toSet();
  ...
  void unfoldAll() {
    final foldedLines = _code.foldedBlocks
        .map((block) => block.firstLine)
        .toList(growable: false);
    for (final line in foldedLines) {
      unfoldAt(line);
    }
  }
  ...
  void setTabSpaces(int value) {
    final normalized = value.clamp(1, 8);
    if (params.tabSpaces == normalized) {
      return;
    }
    params.tabSpaces = normalized;
    notifyListeners();
  }
