...
class GutterWidget extends StatelessWidget {
  ...
  void _fillLineNumbers(List<TableRow> tableRows) {
    final code = codeController.code;
    final selectedLine = _selectedLineIndex();
    ...
    for (final i in code.hiddenLineRanges.visibleLineNumbers) {
      ...
      final isActiveLine = selectedLine == i;
      tableRows[lineIndex].children![_lineNumberColumn] = Container(
        color: isActiveLine ? style.activeLineBackground : null,
        child: Text(
          style.showLineNumbers ? '${i + 1}' : ' ',
          style: isActiveLine
              ? (style.activeLineTextStyle ?? style.textStyle)
              : style.textStyle,
  ...
  void _fillFoldToggles(List<TableRow> tableRows) {
    ...
    for (final block in code.foldableBlocks) {
      ...
      final isActiveLine = _selectedLineIndex() == block.firstLine;

      tableRows[lineIndex].children![_foldingColumn] = FoldToggle(
        color: isActiveLine
            ? (style.activeLineTextStyle?.color ?? style.textStyle?.color)
    ...
    for (final block in code.foldedBlocks) {
      ...
      tableRows[lineIndex].children![_foldingColumn] = FoldToggle(
        color: style.activeLineTextStyle?.color ?? style.textStyle?.color,
        isFolded: true,
  ...
  int? _selectedLineIndex() {
    ...
    if (!selection.isValid || selection.extentOffset < 0) return null;

    final visibleOffset = selection.extentOffset.clamp(
      0,
      codeController.text.length,
    );
    final recoveredOffset = codeController.code.hiddenRanges.recoverPosition(
      visibleOffset,
      placeHiddenRanges: TextAffinity.downstream,
    );
    return codeController.code.lines.characterIndexToLineIndex(recoveredOffset);
  }
}
