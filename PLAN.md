...
- [ ] 在 `AppDetailScreen` / `PostDetailScreen` / `ProjectDetailScreen` 加统一“分享”入口（右上角 Share icon）
- [ ] 统一生成分享链接策略（优先：现有 Web URL；后续：App Deep Link），并把“复制链接 / 系统分享”做成通用组件（例如 `widgets/share_sheet.dart`）
...
- [ ] `DeploymentLogScreen` 增加：复制全部日志 / 复制错误片段 / 系统分享（用于发给同事或支持）
