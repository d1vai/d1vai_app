...
5. 强化 Chat 入口与“继续工作”的主路径
   ...
   - 实施清单：
     ...
     - [x] 全局 Chat Playground（不绑定项目：作为全局入口 + 可选 autoprompt，选择项目后直达 Chat）
...
- [x] 增加“从分享链接打开 app”的深链方案（识别常见 d1v.ai/d1vai.com URL → `go_router` 跳转；未命中则外部浏览器）
...
- [x] 增加“我的帖子 / 我的草稿”列表入口（可挂在 Profile 或 Community 顶部筛选）
- [x] 支持“发布失败重试 + 本地草稿保存”（复用 `models/outbox.dart` 的队列思想或新增轻量 Draft 存储）
- [x] 增加“举报/屏蔽”入口（先只走前端 UI + 提示；后续接后端）
...
- [x] 增加“账号注销/数据导出”入口占位（至少有清晰预期与跳转到 docs/legal）



### 9. 监控与分析（Analytics / Realtime）

- [x] 在 `ProjectAnalyticsTab` 增加筛选：时间范围（7/30/90）、环境（dev/prod）、关键指标切换（errors/requests/latency…按现有 API 能力逐步打开）
- [x] 在 Realtime 页增加“异常提示卡”：当错误率/延迟飙升时，提供快捷跳转（Deploy Logs / Ask AI / Env Vars）
- [x] 增加“导出/分享报表摘要”（复制文本或 CSV；优先做复制摘要）
- [x] 把 Dashboard 的 prompt heatmap 增加“按项目过滤/跳转到项目详情”入口（减少孤立指标）
